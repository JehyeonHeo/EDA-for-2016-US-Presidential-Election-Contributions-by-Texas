...
```
  ...
  Clinton got the most number of contributions. After her, Cruz, Sanders, Trump, Carson, etc. follow. When change the counts to percentages, Clinton got about 38% and Cruz, Sanders, Trump and Carson followed by about 25%, 15%, 14%, 5% each. I think that I need to see if a candidate gets more contributions, the sum of contribtion amounts for the candidate is bigger.
...
```{r echo=FALSE, Bivariate_for_amount_and_date1}
...
```{r echo=FALSE, Explore_more_about_worst_collapsion_date}
...
print('When did the worst collapsion happend?')
...
```{r echo=FALSE, Bivariate_for_amount_and_date2}
...
```{r echo=FALSE, Explore_more_about_March2016_top_0.05}
...
```
 ...
 Fourth plot is scatter plot whose y axis is transformed to log scale. I can see more clearly about lines of dots. And I can also find that from about August, 2015 lots of contributions are done with amounts of \$10 to \$300.
...
```{r echo=FALSE, Bivariate_for_amount_and_party1}
...
```
 ...
 I can see from first boxplot that lots of outlier amounts, especially of which the values are more than \$10,000 are for Republican party candidates. To see more clearly IQR range without outliers, I limited the result from -\$100 to \$600 in second boxplot. And it gave me interesting result. 
 
```{r echo=FALSE, Bivariate_for_amount_and_party2}
...
```
 ...
 The mean and median amounts for Independent candidate and Libertarian party are bigger than the others. Even their IQR is large. But they almost don't have outliers. Considering the fact that the almost all of the contributions are for Democratic and Republican parties which I found in univariate analysis, it looked like the contributions for independent candidate and Libertarian party are done with larger amounts at a time than for the 2 main parties. The mean and median amounts for Green party are slightly bigger than for Republican party. And Democratic party got smallest mean and median. It is interesting that the mean, median of the party are less than the values of Republican party even though the number of contributions for the party are biggest. 

```{r echo=FALSE, Bivariate_for_amount_and_party3}
...
```
 ...
 And third plot shows that the sum of contribution amounts for Republican party was bigger than for Democratic party. And compared to the 2 parties, what others got were really few.
...
```{r echo=FALSE, Bivariate_for_amount_and_candidates1}
...
```
 ...
 In first boxplot, the outlier amounts for Cruz are outstanding. Except for it, I can see that there are lots of outliers for Carson, Clinton, Paul, Sanders and Trump. And the distribution of the amounts for Jeb Bush is also interesting because IQR is large but there are some outlier amounts.
 
```{r echo=FALSE, Bivariate_for_amount_and_candidates2}
...
```
 ...
 I limited from -\$100 to \$3,000 amounts in second plot. And I also showed the mean and median amounts for all candidates arranged by means. The mean and median for Clinton and Sanders are smallest. But they have lots of outliers. Jeb Bush got second largest IQR. The distribution for Carson, Cruz, Paul, Rubio show that the means and medians are small compared to the others, but there are lots of outliers.

```{r echo=FALSE, Bivariate_for_amount_and_candidates3}
...
```
 ...
 I can see from first boxplot that there are a lot more outliers for P2016 than G2016. Without outliers in second boxplot, I can see that the mean and median of P2016 values is bigger than of G2016. In the barplot of the sum of amounts by eletion types I can see that the sum for P2016 is more than 2 times of the sum for G2016.
...
```{r echo=FALSE, Bivariate_for_amount_and_city1}
...
```
 ...
 First plot is boxplot which shows the distribution of amounts in each top 10 city. And I think I can see lots of outlier amounts in Houston and Dallas.
 
```{r echo=FALSE, Bivariate_for_amount_and_city2}
...
```{r echo=FALSE, Explore_more_about_Dallas_top_0.05}
...
```
 ...
 I can see that almost all of top 5% contributions from Dallas were \$2,700. And there were really small counts for the amounts less than \$2,700 from Dallas which I could expect a lot more from the other cities. It looked like that is why their 0.95 quantile was biggest.
 And I can see from the scatter plot that the contributions are distributed to several candidates, not just to 2 or 3.
 
```{r echo=FALSE, Bivariate_for_amount_and_city3}
...
==================================================
 ...
 In this time, I'm going to draw bivariate graphs taking support features. I want to know more about the political difference between cities in Texas.
 ...
 But before I'm going to draw the plot, I'm going to make a variable. Its one column contains the names of candidates and the other column contains the counts of contributions. And there will be only 7 names because I chose to select only 7 main candidates by contribution counts.
...
```
 ...
 It also gave me really funny result. I can see that Clinton got the most contributions from all cities except Spring. 4th place for Trump is also same to almost all top 10 cities. And the counts of contributions for Rubio, Bush, Carson are almost same. Their values are really small compared to the other 4's. But second place changes by each city. In Austin Sanders got gar more contributions than Cruz. But in Dallas and San Antonio Sanders got almost same to Cruz. And the relation became reversed in Houston. The result makes me wonder how the distribution for each party would be different by each city. I think that I will see the huge gap of Democratic party and Republican party in Austin, and smaller gap in Houston.
...
========================================================== 
 ...
 In the above part, I drew bivariate graphs related with cities.
 
 In this time, I'm going to draw bivariate graphs related with dates. And I chose dates because I think that contribution date is useful to let me know about the trends.
...
```
 ...
 I can see that until about June, 2016 lots of contributions are for candidates who weren't nominees. It is likely because at the time nominees weren't decided yet. But the contributions for nominees increased from about February, 2016 and soared highly at about July, 2016. In this time the contributions for candidates who weren't nominees decreased to almost 0. But overall contributions for nominees decreased until about September, 2016 and again increased until the general eletion. I wonder what happend at September, 2016 that the contributions are decreased.
...
```
 ...
 I can see that there are a lot more dramatic fluctuations of contributions for Republican party than for Democratic party. The contributions for Republican party increased more highly and decreased more deeply but the counts are generally more than the counts for Democratic party. But I can see that the contributions for Democratic party catched up almost to the contributions for Republican party at about March, 2016. And I can also see some terms that the contributions for Democratic party are more than for Republican party. They were about from May to June of 2016 and August to November of 2016. I guess that it is somewhat because of Trump. I think that I can find more about it when I draw the plots of dates and candidates. 
 
 In this time, I want to know how the contribution counts changed as time goes on. And I especially want to compare between Trump and Clinton. I chose the 2 candidates because they became main parties' nominees. I'm going to use contb_receipt_dt and cand_nm columns. And I'll use line graph again.
...
```
 ...
 I can see that the part of from June to Noverber of 2016 is almost same to the above date vs party graph. I think that it should be because Clinton and Trump were elected as nominees at July, 2016. 
...
```{r echo=FALSE, Explore_more_about_fluctuation_contbs_for_Trump}
...
```
 ...
 In the graph of dates vs contribution counts for Trump and Clinton, I cannot get the explanation about the fall of contributions to Republican party in May, 2016. Therefore in this time, I'm going to compare Republican party candidates. And I decided to use main_cands_counts to get only main candidates' names.
...
```
 ...
 As I expected, the division point of the contributions for election types occured at about July, 2016. And I can see that this graph is really similar to the graph whose variables were dates and if_nominee except May to August of 2016 term.
...
### Talk about some of the relationships you observed in this part of the investigation. How did the feature(s) of interest vary with other features in the dataset?
 ...
 After I drew the date vs amount graph, I could see that there was a date when really huge collapsion of amounts happened. And I found that it was because of contributions to Cruz by exploring more. I also saw that there were some lines of dots in scatter plot. It means that there are some contribution amounts which were common.
 
 As I drew plots using if_nominee, cand_party, cand_nm, election_tp variables with amount one by one, I could see that lots of outliers are for primary election and for Cruz, Republican candidate who couldn't be elected as a nominee. And I also found that the mean and median amounts for Democratic party were smallest compared to the other parties. But Clinton, Democratic party nominee got the most contribution amounts. Cruz and Trump got second and third most amounts.
 
 As I drew plots using contb_city with amount, I found that the city whose mean of the amounts was biggest was Dallas. 0.95 quantile amount of Dallas was exceptianally big. But when I got the sum of amounts by city, the sum of Dallas was smaller than Houston.
 
### Did you observe any interesting relationships between the other features (not the main feature(s) of interest)?

 When I drew plots for contb_city vs cand_party and contb_city vs cand_nm, it was really interesting that there were really big difference between Houston and Austin. The sum of contribution amounts for Clinton and Sanders was really huge compared to Cruz and Trump in Austin. And even though the sum of contributions in Houston are bigger for Democratic party than for Republican party, the latter value was higest compared to the other cities. And I could find that it was because of the contributions for Cruz. It was interesting to know that there can be huge political contribution differences between cities in the same state.
...
```{r echo=FALSE, Multivariate_Plot_for_amt_dt_city_party}
