...
```
 ...
 When I see the first boxplot, I can find that lots of outlier amounts are for candidates who weren't nominees. To compare the distribution without outliers, I drew second boxplot limiting the amounts to middle 90% values. And I also mark symbol of mean to compare between mean and median. And I can see that IQR is almost similar to each group. But the mean of contribution amounts for nominees are a little bigger than the others and the result of median was in reverse. I can find from barplot that the sum of amounts for nominees are a little bigger than the others.
...
```
 ...
 I can see from first boxplot that lots of outlier amounts, especially of which the values are more than \$10,000 are for Republican party candidates. To see more clearly IQR range without outliers, I limited the result to middle 90% amounts in second boxplot. And it gave me interesting result. 
 
 The mean and median amounts for Independent candidate and Libertarian party are bigger than the others. Even their IQR is large. But they almost don't have outliers. I found that the almost all of the contributions are for Democratic and Republican parties in univariate analysis. Considering it, it looked like the contributions for independent candidate and Green, Libertarian parties are done with larger amounts at a time than for the 2 main parties. Even the mean for Green party are bigger than for Republican party and Democratic party got smallest mean. I also find that the number of contributions for Democratic party are biggest and mean, median of the party are less than the values of Republican party. 
...
```
 ...
 I limited to middle 90% amounts in second plot. The mean and median for Clinton and Sanders are smallest. But they have lots of outliers. Jeb Bush got second largest IQR. The distribution for Carson, Cruz, Paul, Rubio show that the means and medians are small compared to the others, but there are lots of outliers.
...
# I used again aes(shape = 'mean') and scale_shape_manual to show mean of each group.
ggplot(aes(x = election_tp, y = contb_receipt_amt),
       data = subset(subset_amounts_within_middle_90_percent,
                     election_tp != '')) +
  geom_boxplot(alpha = 0.2) +
  geom_point(stat = 'summary', fun.y = mean, size = 2, aes(shape = 'mean')) +
  scale_shape_manual('', values = c('mean' = 8)) +
...
```
 ...
 I can see from first boxplot that there are a lot more outliers for P2016 than G2016. But without outliers in second boxplot, I can see that the medians and IQRs are almost same. But the mean of G2016 values is bigger than of P2016s. But in the barplot of the sum of amounts by eletion types I can see that the sum for P2016 is more than 2 times of the sum for G2016.
...
date_vs_main_nominees + scale_x_date(limits = c(as.Date('2016-04-01'),
                                                as.Date('2016-11-30')),
                                     date_breaks = '1 month')
...
```
 ...
 I can see more in detail by limiting the term form April to November of 2016. 
There are lots of huge fluctuations for Trump. I wonder if it is related with the comments and news of Trump. In contrast to Trump, Clinton got more and more contributions generally as time goes on.
...
date_vs_main_democratics + scale_x_date(date_breaks = '3 month')

date_vs_main_democratics + scale_x_date(limits = c(as.Date('2016-04-01'),
                                                as.Date('2016-11-30')),
                                     date_breaks = '1 month')
```

 I can find that there were some time when Sanders got more contributions than Clinton. I wonder why he became a lot more support from Texas than Clinton at the time. Anyway it explains why the contributions for Democratic party catched up the those for Republican party at about March, 2016.
 
 In this time, I want to know how the contribution counts for each election type changed as time goes on. Therefore I'm going to use contb_receipt_dt and election_tp columns. And I'll again use line graph. I expect that division between the types are almost flat at about July, 2016 because at that time party primary elections happened.
...
```
 ...
 As I expected, the division point of the contributions for election types occured at about July, 2016.
